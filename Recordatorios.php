...
// Configuración: cuántos días antes enviar el recordatorio
$dias_anticipacion = 1; // 1 día antes

$appointmentModel = new Appointment();
$userModel = new User();

// Calcular la fecha objetivo (mañana si es 1 día de anticipación)
$fecha_recordatorio = date('Y-m-d', strtotime("+{$dias_anticipacion} days"));

echo "[" . date('Y-m-d H:i:s') . "] Buscando citas para: $fecha_recordatorio\n";

// Obtener todas las citas pendientes o confirmadas para esa fecha
$query = "SELECT a.*, u.name, u.email, u.phone, p.name as pet_name, p.breed 
          FROM appointments a
          INNER JOIN users u ON a.user_id = u.id
          INNER JOIN pets p ON a.pet_id = p.id
          WHERE a.appointment_date = :fecha
...
echo "Citas encontradas: " . count($citas) . "\n\n";
...
foreach ($citas as $cita) {
    ...
    // Asunto del email
    $subject = "🐶 Recordatorio: Cita en Dog Cute Spa mañana";
    ...
    <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;'>
        ...
        <div style='padding: 30px; background: #f8f9fa;'>
            ...
            <p style='font-size: 16px; color: #555;'>
                Te recordamos que <strong>{$nombre_mascota}</strong> tiene una cita programada para <strong>mañana</strong>:
            </p>
